/*
 * $Id: udp_server.c,v 1.2 2003/12/13 20:59:19 jiri Exp $
 *
...
int probe_max_receive_buffer( int udp_sock )
{
	...
	if ( ioptval==0 ) 
	{
		LOGL(L_DBG, "DEBUG: udp_init: SO_RCVBUF initialy set to 0; resetting to %d\n",BUFFER_INCREMENT );
...
*/
	...
	if (init_su(addr, &sock_info->address, sock_info->port_no)<0){
		LOGL(L_ERR, "ERROR: udp_init: could not init sockaddr_union\n");
		goto error;
	}
	
	sock_info->socket = socket(AF2PF(addr->s.sa_family), SOCK_DGRAM, 0);
	if (sock_info->socket==-1){
		LOGL(L_ERR, "ERROR: udp_init: socket: %s\n", strerror(errno));
		goto error;
	}
	/* set sock opts? */
	optval=1;
	if (setsockopt(sock_info->socket, SOL_SOCKET, SO_REUSEADDR ,
		...
		LOGL(L_ERR, "ERROR: udp_init: setsockopt: %s\n", strerror(errno));
		goto error;
	}
	/* tos */
	...
	if (setsockopt(sock_info->socket, IPPROTO_IP, IP_TOS, (void*)&optval, 
			sizeof(optval)) ==-1){
		LOGL(L_WARN, "WARNING: udp_init: setsockopt tos: %s\n", strerror(errno));
	...
	if (bind(sock_info->socket,  &addr->s, sockaddru_len(*addr))==-1){
		LOGL(L_ERR, "ERROR: udp_init: bind(%x, %p, %d) on %s: %s\n",
...
int udp_rcv_loop()
{
	...
	*/
	...
	for(;;){
		...
		if (len==-1)
		{
			LOGL(L_ERR, "ERROR: udp_rcv_loop:recvfrom:[%d] %s\n",errno, strerror(errno));
...
again:
	...
	LOGL(L_INFO, "INFO: send status: %d\n", n);
	if (n==-1){
		LOGL(L_ERR, "ERROR: udp_send: sendto(sock,%p,%d,0,%p,%d): %s(%d)\n",buf,len,to,tolen,strerror(errno),errno);
		if (errno==EINTR) goto again;
		if (errno==EINVAL) 
                {
			LOGL(L_CRIT,"CRITICAL: invalid sendtoparameters\n"
