<<<<<<< ip_addr.c
/*
 * $Id: ip_addr.c,v 1.1 2003/12/19 00:24:45 gabriel Exp $
 *
...
struct net* mk_net(struct ip_addr* ip, struct ip_addr* mask)
{
	...
	if ((ip->af != mask->af) || (ip->len != mask->len)){
		LOG("ERROR: mk_net: trying to use a different mask family"
				" (eg. ipv4/ipv6mask or ipv6/ipv4mask)\n");
		goto error;
	}
	n=(struct net*) malloc(sizeof(struct net));
	if (n==0){ 
		LOG("ERROR: mk_net: memory allocation failure\n");
		goto error;
	}
	n->ip=*ip;
	n->mask=*mask;
...
struct net* mk_net_bitlen(struct ip_addr* ip, unsigned int bitlen)
{
	...
	if (bitlen>ip->len*8){
		LOG("ERROR: mk_net_bitlen: bad bitlen number %d\n", bitlen);
		goto error;
	}
	n=(struct net*) malloc(sizeof(struct net));
	if (n==0){
		LOG( "ERROR: mk_net_bitlen: memory allocation failure\n"); 
		goto error;
	}
...
{
	if (net==0){
		LOG( "ERROR: print net: null pointer\n");
		return;
	}
